...
// Converte variant para string
std::string variantToString(const std::variant<int, float, double, long, char*, char, std::string>& var) {
...
// Faz o erro
void error(const std::variant<int, float, double, long, char*, char, std::string>& error) {
    std::cerr << variantToString(error);
}

// Converte string para uppercase
static void to_uppercase(const char* in, char* out) {
    for (int i = 0; in[i]; ++i) out[i] = static_cast<char>(std::toupper(static_cast<unsigned char>(in[i])));
    out[std::strlen(in)] = '\0';
}
...
extern "C" {
    ...
    __declspec(dllexport) void write_register(const char* reg, uint32_t v) {
        ...
        error(up + "it's not supported by architeture");
    ...
    __declspec(dllexport) void read_register(const char* reg) {
    #else
    ...
    __declspec(dllexport) const char* execute() {
